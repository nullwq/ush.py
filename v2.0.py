...
def run_c(h,p):
    ...
    def poll_size():
        old=(os.get_terminal_size() if IS_WIN else struct.unpack("HH",fcntl.ioctl(0,21523,b'\x00'*8)))
        while not stop[0]:
            time.sleep(0.5)
            new=(os.get_terminal_size() if IS_WIN else struct.unpack("HH",fcntl.ioctl(0,21523,b'\x00'*8)))
            if new!=old:
                old=new
